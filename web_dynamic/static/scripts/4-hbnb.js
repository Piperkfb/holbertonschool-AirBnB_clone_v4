...
function getCheckedAmensId () {
  let checked = [];
  amenitiesCheck = $('input[type=checkbox]');

  Object.values(amenitiesCheck).forEach((amen) => {
    if (amen.checked) {
      checked.push(amen.dataset.id);
    }
  });
  checked = checked.length === 0 ? {} : { 'amenities': checked };
  return checked;
}
...
$(document).ready(function () {
  ...
  search.click(() => {
    $.post({
      ...
      data: JSON.stringify(getCheckedAmensId()),
      contentType: 'application/json',
      dataType: 'json'
    })
      .done((places) => {
        const placesSect = $('section.places');
        if (placesSect.children.length > 0) {
